...
- **Nível-capacidade (alcance GLOBAL):** usuário com `aprovar` em **qualquer** módulo de negócio **não pode** ter `editar`/`criar`/`inativar` em **nenhum** módulo de negócio — **nem via override**. Definição canônica em [`01-RBAC-PERFIS.md`](01-RBAC-PERFIS.md) **§4.1.1** (fonte única; não redefinir aqui). Validado a cada mudança de perfil/override sobre o conjunto **efetivo**, fail-closed. *(Correção D5, 2026-07-24: dizia "no mesmo módulo" — os módulos do projeto são etapas da MESMA cadeia de despesa, e por módulo autorizaria editar o empenho e aprovar o pagamento. Validação **no boot** é pendente de verificação, não presuma implementada.)*
